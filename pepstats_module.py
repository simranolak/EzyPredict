...
#
# EMBOSS pepstats (REST) web service Python client using xmltramp2.
#
...
Asynchronous job:
  ...
  Usage: python emboss_pepstats.py --asyncjob --email <your@email.com> [options...] <SeqFile|SeqID(s)>
  Returns: jobid
...
baseUrl = u'https://www.ebi.ac.uk/Tools/services/rest/emboss_pepstats'
...
# Submit job
    ...
    if not options.termini:
        params['termini'] = 'true'
    if options.termini:
        params['termini'] = options.termini
    

    if not options.mono:
        params['mono'] = 'false'
    if options.mono:
        params['mono'] = options.mono
